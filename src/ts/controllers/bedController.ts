...
export class BedController extends BaseController {
  ...
  protected onActivate() {
    this.newTarget = this.currentTarget || 60;
    this.setValue();
  }
  ...
  public save() {
    this.pm.execute({action: 'serial', method: 'gcode', command: 'M140 S' + this.newTarget});
  }
  public update(data: {[key: string]: any}) {
    if (data) this.currentTarget = data['heatbed_t'];
  }
}